...
- baseUrl: https://auth1.sandbox.tccl.uk-hub-prod.ozoneapi.co.uk/

### Step by Step guide to connect to sandbox

#### Step 1: Pre-Requisites for TPP
Ensure that the following pre-requisites are met before onboarding onto Currencycloud.
...
| iat | timestamp |
| jti |  |  UUID
| response_types | code, code id_token |  JSON array containing a list of the OAuth 2.0 response_type values that the Client is declaring that it will restrict itself to using
...
response = requests.post("https://ob-api1.sandbox.tccl.uk-hub-prod.ozoneapi.co.uk/dynamic-client-registration/v3.1/register",
...
Once a TPP successfully registers to Currencycloud, they can download the environment file for their client via an Currencycloud helper api

```
Endpoint: https://ob-api1.sandbox.tccl.uk-hub-prod.ozoneapi.co.uk/ozone/v1.0/postman/environment
...
https://rs1.sandbox.tccl.uk-hub-prod.ozoneapi.co.uk
https://auth1.sandbox.tccl.uk-hub-prod.ozoneapi.co.uk/token
...
Once the PSU consent is successful, Currencycloud will redirect back to the redirect URI. Copy the Authcode from the URL
